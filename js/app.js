...
app.controller('translateContrl',[ '$http','$scope', function ($http,$scope){
    ...
    var baseURL = 'https://www.googleapis.com/language/translate/v2?key=AIzaSyCSkJzCc7-jPArWHMYCeWSaIstDTzO7iYY';
    var translatedword='';
    $scope.show="";
    ...
    $scope.translatesearch = function(target,word){
        ...
        $http.get(baseURL+'&target='+target+'&q='+word).then(function(response){
    ...
    $scope.translate = function(target,word){
        return $http.get(baseURL+'&target='+target+'&q='+word);
    }

    $scope.showTranslate = function(r) {
        return $http.get(baseURL+'&target=en&q='+r.title);
    }
